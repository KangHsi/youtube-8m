...
from data_io import hdfs_downloader as hdfs
import threading
...
if __name__ == "__main__":
  ...
  # hdfs downloader flags
  flags.DEFINE_string("data_tmp_dir", "tmp",
                      "Temp dir for saving hdfs data")
  flags.DEFINE_integer("num_downloaders", 1,
                      "Number of downloaders to download file from HDFS")
  flags.DEFINE_integer("enqueue_size", 4,
                       "Number of files to enqueue into data queue each time")
...
def create_hdfs_downloader(data_pattern,
                           data_tmp_dir,
                           num_downloader=1,
                           enqueue_size=4):
    downloader = hdfs.HDFSDownloader(data_pattern, data_tmp_dir,
                            min_cache=enqueue_size,
                            num_downloader=num_downloader)
    logging.info('Start {} HDFS downloader'.format(num_downloader))
    downloader.start_downloading()
    logging.info("Number of training files: %s.", str(downloader.num_files))
    return downloader
...
def get_input_evaluation_tensors(reader,
  ...
  with tf.name_scope("eval_input"):
    filename_queue = downloader.get_out_queue()
    eval_data = [
...
def evaluation_loop(downloader, video_id_batch, prediction_batch, label_batch, loss,
  ...
  with tf.Session(config=config) as sess:
    ...
    # start downloading
    enqueue_thread = threading.Thread(target=downloader.enqueuing, args=(sess,))
...
def evaluate():
  ...
  with tf.Graph().as_default():
    ...
    # create HDFS downloader
    downloader = create_hdfs_downloader(FLAGS.eval_data_pattern,
                        FLAGS.data_tmp_dir,
    ...
    while True:
      last_global_step_val = evaluation_loop(downloader, video_id_batch, prediction_batch,
