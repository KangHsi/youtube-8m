...
#                           task_as_string(self.task))
  ...
  def run(self, start_new_model=False):
    ...
    with tf.Graph().as_default() as graph:
      ...
      self.downloader = create_hdfs_downloader(FLAGS.train_data_pattern, FLAGS.data_tmp_dir,
                          num_downloader=FLAGS.num_downloaders,
                          enqueue_size=FLAGS.enqueue_size)

      with tf.device(device_fn):
        if not meta_filename:
          saver = self.build_model(self.model, self.reader, self.downloader)
  ...
  def build_model(self, model, reader, downloader):
    ...
    return tf.train.Saver(max_to_keep=20, keep_checkpoint_every_n_hours=0.5)
